class MediumLevel:
    def getDirection(self, pos, dir):
        try:
            ...
            for s in file:
                if len(dir)==2:
                    if s[0]==pos[0] and s[1]==pos[1] and s[3]==dir[0] and s[4]==dir[1]:
                        return s[6:len(s)-1]
                else:
                    if s[0]==pos[0] and s[1]==pos[1] and s[3]==dir[0]:
                        return s[6:len(s)-1]
        except:
            print("Error occured in MediumLevel getDirection")
        finally:
            file.close()
        return None 
    ...
    def getDescription(self, s):
        try:
            ...
            print("Error occured in MediumLevel getDescription")
        finally:
            file.close()
        return None